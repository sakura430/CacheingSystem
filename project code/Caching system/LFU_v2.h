...
namespace cache_set{
template<typename Key,typename Value> class LFU_V2;
...
public:
    ...
    LFU_V2(int capacity,int max_ave = 1000000)
    :_capacity(capacity),
...
void LFU_V2<Key,Value>::get_internal(Node_ptr node,Value value){
    ...
    node -> _fre_times++;
    add2Fre_list(node);
    if(node -> _fre_times -1 == _fre_min && fre2fre_list[node->_fre_times -1] -> is_empty()){
        _fre_min++;
    }
    add_fre_times();
    return ;
...
void LFU_V2<Key,Value>::kick_out(){
    ...
    removeFromFre_list(node);
    _Node_map.erase(node->_key);
    dec_fre_times(node->_fre_times);
    return ;
}
...
class Hash_LFU_V2{
public:
    Hash_LFU_V2(size_t capacity,int slice_nums,int max_ave_num = 10):
    _capacity(capacity),
    _slice_nums(slice_nums > 0 ? slice_nums : thread::hardware_concurrency()){
        ...
        size_t slice_size = std::ceil(capacity / static_cast<double>(_slice_nums));
