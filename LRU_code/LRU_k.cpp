...
private:
    ...
    void add_new_node(const Key & key,const Value & val){
        node_ptr newNode = make_shared<Key(),Value()>;
        /*if(Lru_list.size() == _capacity){
            evict_leastRecent();
        }*/
...
public:
    ...
    _slice_nums(slice_nums > 0? slice_nums : hardwear_concurrency()){
